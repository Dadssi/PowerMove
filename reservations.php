...
<body class="text-gray-800">
    ...
    <!-- ------------------------------------------ -->
    <div class="flex flex-col gap-4 overflow-x-auto p-4 bg-white rounded-lg shadow-md mb-44">
    <div class="w-full">
        <h2 class="text-lg font-bold text-purple-700 mb-2">Listes des Réservations</h2>
        <table class="table-auto w-full border-collapse border border-gray-200">
            <thead class="bg-purple-700 text-white">
                <tr>
                    <th class="px-4 py-2 border border-gray-300">ID Réservation</th>
                    <th class="px-4 py-2 border border-gray-300">ID Membre</th>
                    <th class="px-4 py-2 border border-gray-300">Activité</th>
                    <th class="px-4 py-2 border border-gray-300">Date de Réservation</th>
            ...
            <tbody>
                <?php foreach ($reservations as $reservation): ?>
                    <tr class="odd:bg-purple-50 even:bg-purple-100 hover:bg-purple-200">
                        <td class="px-4 py-2 border border-gray-300"><?= htmlspecialchars($reservation['id_reservation']) ?></td>
                        <td class="px-4 py-2 border border-gray-300"><?= htmlspecialchars($reservation['id_membre']) ?></td>
                        <td class="px-4 py-2 border border-gray-300"><?= htmlspecialchars($reservation['id_activite']) ?></td>
                        <td class="px-4 py-2 border border-gray-300"><?= htmlspecialchars($reservation['date_reservation']) ?></td>
                        <td class="px-4 py-2 border border-gray-300"><?= htmlspecialchars($reservation['statut']) ?></td>
